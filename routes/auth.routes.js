...
router.post(
  ...
  [
    ...
    check('login', 'Минимальная длина login-а 3 символа').isLength({ min: 3 }),
  ...
  try {
    ...
    const { email, password, login, name, secondName } = req.body;
    ...
    const user = new User({ email, login, name, secondName, password: hashedPassword });
    await user.save();
